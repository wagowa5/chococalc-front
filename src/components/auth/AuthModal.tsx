...
const AuthModal = ({
  ...
  const handleGeneratePassword = () => {
    ...
    dispatch({
      type: AUTH_MODAL_ACTIONS.SET_PASSWORD,
  ...
  const handleLogin = () => {
    ...
    cognitoUser.authenticateUser(authenticationDetails, {
      onSuccess: (session) => {
        ...
        dispatch({
          type: AUTH_MODAL_ACTIONS.SET_VERIFICATION_CODE,
          payload: '',
        });
      ...
      onFailure: (err) => {
        ...
        dispatch({
          type: AUTH_MODAL_ACTIONS.SET_AUTH_STATUS,
          payload: MESSAGES.AUTH_MODAL_KEYS.LOGIN_ERROR,
        });
        setCognitoUser(null);
      },
    });
    ...
    userPool.signUp(
      ...
      (err, result) => {
        if (err) {
          dispatch({
            type: AUTH_MODAL_ACTIONS.SET_AUTH_STATUS,
            payload: MESSAGES.AUTH_MODAL_KEYS.SIGNUP_ERROR,
          });
          return;
        }
        // サインアップ成功時の処理
        dispatch({
          type: AUTH_MODAL_ACTIONS.SET_AUTH_STATUS,
          payload: MESSAGES.AUTH_MODAL_KEYS.SIGNUP,
        });
      },
    );
  };
    ...
    cognitoUser.confirmRegistration(
      ...
      function (err, result) {
        if (err) {
          ...
          dispatch({
            type: AUTH_MODAL_ACTIONS.SET_AUTH_STATUS,
            payload: MESSAGES.AUTH_MODAL_KEYS.VERIFICATION_ERROR,
          });
          return;
        }
        ...
        dispatch({
          type: AUTH_MODAL_ACTIONS.SET_AUTH_STATUS,
          payload: MESSAGES.AUTH_MODAL_KEYS.VERIFICATION_SUCCESS,
        });
      },
    );
  };
            ...
            <Grid container spacing={1} margin={0}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: AUTH_MODAL_ACTIONS.SET_EMAIL,
